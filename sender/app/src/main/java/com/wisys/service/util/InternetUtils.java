package com.wisys.service.util;
...
public class InternetUtils  {
    ...
    public void init() {
        ...
        try{
            controlSocket = new Socket(serverAddr, controlPort);
    ...
    public String readCommand() throws IOException {
        ...
        while((len = controlInStream.read(bytes)) != -1) {
            controlSB.append(new String(bytes, 0, len, StandardCharsets.UTF_8));
        ...
        matcher.find();
        dataSize = Integer.parseInt(matcher.group(2));
    ...
    public byte[] readData() throws IOException {
        ...
        while(totalLen < dataSize && (len = dataInStream.read(bytes)) != -1) {
            if (len >= 0) System.arraycopy(bytes, 0, result, totalLen, len);
            totalLen += len;
        }
        return result;
    }
